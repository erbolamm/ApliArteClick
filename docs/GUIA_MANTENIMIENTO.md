...
## 🪟 Proceso de Lanzamiento (Windows)

Para generar la versión oficial de Windows:
...
2. **Generar Instalador (MSIX)**:

    ```bash
    flutter pub run msix:create
    ```

3. **Empaquetar Manual (ZIP)**:
    - Comprime la carpeta `build/windows/runner/Release`.
    - Renómbrala como `ApliArteClickPro-Windows-v3.0.0.zip`.

4. **GitHub Release**:
    - Sube el archivo `.zip` y/o `.msix` al release correspondiente.
